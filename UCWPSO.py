...
class UCWPSO:
    # c1 = c2 = 2; w = 0.8; maxIter = 50; popSize = 30
    ...
    def processingEstimated(popSize, dataPoints, positions):
        ...
        for i in range(popSize):
            estimatedEffort = UCWPSO.estimatingUCP(dataPoints, positions[i][0], positions[i][1], positions[i][2])
    ...
    def optimizing(dataPoints, popSize, seeds):
        estimatedEfforts = [] 
        bestPosition = []
        absoluteErrors = [] 
        actualEffortIndex = 6
        ...
        #initialPopulation = UCWPSO.initializingPopulation(dataPoints, popSize)
        initialPopulation = seeds
        
        for l in range(popSize):
            estimatedEffort = UCWPSO.estimatingUCP(dataPoints, initialPopulation[l][0], initialPopulation[l][1], initialPopulation[l][2])
            estimatedEfforts.append(estimatedEffort)
        ...
        for i in range(maxIter):
            ...
            if (i > 0):
                vels = UCWPSO.updateVelocity(particles, r1, r2, popSize)
                positions = UCWPSO.updateParticles(particles[0], vels, popSize)
                positions = UCWPSO.checkLimit(positions)
                
                fitness = UCWPSO.processingEstimated(popSize, dataPoints, positions)
                pbest = UCWPSO.checkPbest(particles[2], particles[4], positions, fitness[1])
                fitness = UCWPSO.processingEstimated(popSize, dataPoints, pbest)
