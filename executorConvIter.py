...
class Executor:
    def __init__(self, dataSet, popSize, maxIter):
    ...
    def executing(self, seeds):
        ...
        for dataPoints in self.dataSet:
            results = CPSO.optimizing(dataPoints, self.popSize, seeds, self.maxIter)
...
for x in range(12):
    for i in range(runs):
        print('RUNS Ke- ', i)
        run = Executor(readSilhavy71(), popSize, maxIter)
    ...
    print('maxIter: ', maxIter, ' MAE: ', finalMAE)

    listToWrites = ['UCW+CPSO Bern', 'MAE: ' + str(finalMAE), 'maxIter: ' + str(maxIter)]
    with open('results.txt', 'a') as f:
        for listToWrite in listToWrites:
            f.write(listToWrite)
            f.write('\n')
    sumMAE = 0; listToWrites = []; finalMAE = []; bestMAEs = []; actualEfforts = []
    maxIter += 5