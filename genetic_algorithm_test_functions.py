...
class GeneticAlgorithm:
    ...
    def oneCutPointCrossover(self, population):
        ...
        cutPointIndex = random.randint(0, self.testFunctionParameters['dimension']-1)
    ...
    def mutation(self, population):
        ret = False
        mutatedValue = 0
        mutationRate = 1 / self.testFunctionParameters['dimension']
        ...
        if numMutation:
            for _ in range(numMutation):
                ...
                selectedGenIndex = random.randint(0, self.testFunctionParameters['dimension']-1)
                while mutatedValue < self.testFunctionParameters['ranges'][0] or mutatedValue > self.testFunctionParameters['ranges'][1]:
                    mutatedValue = random.uniform(
                        self.testFunctionParameters['ranges'][0], 
